...
class EnrollmentAdminForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if classroom and is_active:
            ...
            if self.instance.pk:
                active_enrollments = active_enrollments.exclude(
                    pk=self.instance.pk
                )
            ...
            if (
                ...
                raise forms.ValidationError(
                    ...
                    "برای ثبت‌نام استثنایی، گزینه تأیید ثبت‌نام بیش از ظرفیت را فعال کنید."
                )
        ...
        if existing.exists():
            raise forms.ValidationError(
                "این دانش‌آموز در حال حاضر در این سال تحصیلی یک ثبت‌نام فعال دارد."
            )
